...
Для упрощения этой задачи, назначим тегу монтирования компонента Hello атрибут ***id*** со значением "hello", как показано ниже:
...
Свойство **mode** добавляет компоненту [Теневой DOM](https://learn.javascript.ru/shadow-dom#tenevoe-derevo), который изолирует [внутренние стили](https://learn.javascript.ru/shadow-dom-style) компонента от всего приложения и определяет уровень его инкапсуляции:

```js
mode: 'open'
```

Это свойство может иметь значение "open" или "closed" и, кроме этого, его можно использовать лишь с [некоторыми](https://developer.mozilla.org/ru/docs/Web/API/Element/attachShadow), модифицированными при помощи свойства **extends** компонентами.
...
Все рассмотренные выше свойства объекта компонента, кроме методов и свойства **attributes**, можно передавать через атрибуты элемента TEMPLATE.

В качестве примера, модифицируем этот компонент и добавим ему Теневой DOM с открытым уровнем инкапсуляции:
...
Однако, свойства, которые экспортируются с помощью объекта **exports**, переопределяют свойства задаваемые в атрибутах элемента TEMPLATE.

В примере ниже, мы изменили свойство **name**, которое отвечает за наименование компонента:
...
И поэтому, данный компонент не будет отображаться в браузере, когда мы попытаемся смонтировать компонент Hello:
...
Reacton поддерживает два типа внешних компонентов. Первый тип - это модульные компоненты. Они создаются в виде обычных [модулей](https://learn.javascript.ru/modules-intro) JavaScript и, как правило, имеют расширение *.mjs*. Второй тип внешних компонентов в Reacton - это однофайловые компоненты, которые пишутся на чистом HTML и имеют расширение *.htm*.

После ознакомления с модульными компонентами, все дальнейшие примеры данного руководства, будут показаны в виде однофайловых компонентов.
...
Для работы с модулями JavaScript, потребуется любой разработочный сервер, такой, например, как [lite-server](https://www.npmjs.com/package/lite-server). Кроме этого, данный сервер обеспечивает автоматическую перезагрузку страницы, при любом изменении содержимого каталога, из которого он запущен.
...
Однофайловые компоненты по своей природе напоминают [встроенные](#embedded-components). Единственным исключением является тот факт, что однофайловым компонентам не требуется атрибут ***name***, содержащий название компонента, так как данное название определяется именем корневого тега, в котором содержится компонент.
...
В предыдущем разделе мы создали однофайловый компонент Hello и, как мы помним, для запуска приложения мы теперь используем разработочный сервер. Перейдите из терминала в свой рабочий каталог *app* и введите команду: 
...
Кроме этого, в файле *index.html* мы добавили атрибут ***id*** со значением "hello" тегу монтирования компонента Hello, для быстрого к нему доступа:
...
События задаются через специальный атрибут, который начинается с символа ***@***, за которым следует название события.

Обо всём этом будет подробно рассказано в дальнейших разделах этого руководства. Но вернёмся к специальным свойствам, для проверки работы которых, вы можете воспользоваться любым из рассмотренных ранее способов.

<br>

<h3 id="root"># $root</h3>

Специальное свойство **$root** является сокращённым аналогом стандартного свойства [shadowRoot](https://learn.javascript.ru/shadow-dom#tenevoe-derevo) Веб-компонента. Оно возвращает ссылку на его [Теневой DOM](https://learn.javascript.ru/shadow-dom), если он был добавлен с помощью свойства **mode** объекта компонента:
...
Для понимания работы этого метода, нам придётся временно создать второй компонент в файле *Hello.htm*, как мы это делали в предыдущем разделе:
...
Не забудьте смонтировать компонент Bye в файле *index.html*, как показано ниже:
...
Для вычисления выражений в разметке компонента, в библиотеке Reacton применяется синтаксис шаблонизатора [Mustache](http://websketches.ru/plugins/mustache). Выражения размещаются между двойными фигурными скобками, как показано ниже:
...
Все текстовые [узлы](https://developer.mozilla.org/ru/docs/Web/API/Document_Object_Model/Introduction) и узлы атрибутов, где встречаются выражения, становятся реактивными узлами в Reacton.

Допускается добавлять ключевое слово **this**, перед пользовательским свойством в выражении:
...
Для пользовательских свойств нет необходимости добавлять ключевое слово **this**, однако, для доступа к свойствам самого компонента в разметке, оно является обязательным в выражении.

Давайте получим значение атрибута ***id*** компонента Hello и выведем его в параграфе, как показано ниже:
...
Вместо значения "hello", будет выведено значение "1".
...
Параметры необходимы для передачи любых данных в дочерние компоненты, т.е. такие, которые монтируются в другие компоненты. Перед тем, как рассматривать параметры, давайте посмотрим, как можно передавать текстовые значения через атрибуты в дочерние компоненты.
...
При запуске приложения, на экране браузера будут отображаться два сообщения:
...
Для решения этой задачи, во-первых, можно воспользоваться [наблюдаемыми атрибутами](https://learn.javascript.ru/custom-elements#nablyudenie-za-atributami), предлагаемыми технологией самих Веб-компонентов. Давайте этим и воспользуемся, используя свойство [attributes](#attributes) и метод [changed](#changed), объекта компонента Bye, как показано ниже:
...
В конце экспортируемого объекта, мы используем метод **changed**, который вызывается при изменении любого наблюдаемого атрибута, в котором мы просто присваиваем пользовательскому свойству **message** новое значение:
...
Параметры являются технологией предоставляемой библиотекой Reacton и определяются специальным атрибутом ***$params*** в родительском компоненте, и одноимённым свойством в дочернем.
...
В атрибуте ***$params*** передаются через запятую названия пользовательских свойств родительского компонента, которые будут доступны в дочернем. Давайте снова попробуем повторить пример выше, но теперь используя для этого параметры.
...
<app-hello>
  ...
  <!-- монтировать компонент Bye в приложение
    и передать ему название пользовательского свойства через параметры -->
...
Здесь нет ничего сложного, мы просто передаём в тег монтирования компонента Bye атрибут ***$params***, который содержит название пользовательского свойства **message** из компонента Hello.
...
Специальное свойство **$params** является объектом [прокси](https://learn.javascript.ru/proxy).

Доступ к значениям пользовательских свойств родительского компонента, осуществляется через точечную нотацию.
...
<app-hello>
  ...
  <!-- монтировать компонент Bye в приложение
    и передать ему названия пользовательских свойств через параметры -->
...
Давайте выведем этот массив и объект из родительского компонента Hello, в дочернем компоненте Bye:
...
Это связано с тем, что внутри этой функции нам было необходимо получить доступ к специальному свойству **$params**, которое доступно через ключевое слово **this**.

Метод **data** объекта компонента, который возвращает объект с пользовательскими свойствами и методами, выполняется в контексте компонента, а стрелочные функции, получают значение ключевого слова **this** на месте их определения.
...
Примеси позволяют определять значения для всех компонентов сразу. Например, в примеси можно определить функцию, которая будет доступна во всех компонентах. Примеси определяются с помощью назначения свойства **mixins**, главной функции библиотеки Reacton.
...
Теперь давайте вызовем функцию **print** в компонентах Hello и Bye. Для этого используется специальное свойство **$mixins**.

Внесите изменения в файл *Hello.htm*, в котором содержатся эти два компонента:
...
Вот, собственно, и всё, что касается механизма определения примесей в Reacton. На экране мы увидим уже знакомое нам сообщение:
...
Библиотека Reacton основана на [Веб-компонентах](https://learn.javascript.ru/web-components) и она поддерживает всё, что предоставляет данная технология, включая и [слоты](https://learn.javascript.ru/slots-composition).

Переносить сюда руководство по описанию слотов не имеет смысла. Вы можете ознакомиться с ним по ссылке выше. Но стоит обратить внимание на то, что слоты могут иметь только компоненты, которым был задан [Теневой DOM](https://learn.javascript.ru/shadow-dom) с открытым или закрытым уровнем инкапсуляции.

В Reacton это делается с помощью определения свойства [mode](#mode) в экспортируемом объекте компонента. Данное свойство может иметь значение "open" или "closed" и, кроме этого, его можно использовать лишь с [некоторыми](https://developer.mozilla.org/ru/docs/Web/API/Element/attachShadow) модифицированными при помощи свойства [extends](#extends) компонентами.
...
Данный цикл выведет 11 параграфов с числами от 0 до 10.

Обратите внимание, что в атрибуте ***$for*** нельзя использовать операторы определения переменных *var*, *let* и *const* соответственно.
...
<span id="actual">Здесь</span> стоит отдельно обратить внимание на актуальное взаимодействие событий и переменных циклов компонента.

Даже после обращения массива, для чего введите в консоли браузера команду:
...
Трёхуровневый цикл выше, выводит детальную информацию о каждом пользователе из массива **users**, используя для этого два внутренних цикла.
...
Все специальные атрибуты событий, начинаются в Reacton с символа ***@***, за которым следует название события.

Давайте создадим событие *click* для элемента H1, которое будет изменять значение свойства **message**, как показано ниже:
...
```
Внутри атрибута событий нет необходимости использовать двойные фигурные скобки.

Кроме этого, все атрибуты событий могут иметь параметры, которые задаются через точечную нотацию и соответствуют названиям объекта параметров, передаваемого в третьем аргументе методу [addEventListener](https://learn.javascript.ru/introduction-browser-events#addeventlistener).
...
Булевы или [логические](http://htmlbook.ru/samouchitel-html5/atributy) атрибуты, определяются в Reacton с помощью уже знакомого нам символа доллара ***$*** и следующего за ним названия логического атрибута.

Все специальные атрибуты, которые начинаются со знака доллара, кроме атрибутов циклов и видов, о которых будет рассказано в следующем разделе, воспринимаются в Reacton как логические.

На практике это означает, что если атрибут начинается со знака доллара, но не является ни циклом, ни видом, то это специальный логический атрибут.

При этом, если его значение вычисляется как Истина, то в HTML-элемент, которому он принадлежит, добавляется стандартный логический атрибут.

И наоборот, в случае значения Ложь специального логического атрибута, стандартный логический атрибут удаляется из его HTML-элемента.
...
При нажатии на кнопку Скрыть/Показать, значение пользовательского свойства **hide** будет инвертироваться и, соответственно, заголовок компонента будет скрываться/отображаться на экране.

Другими словами, ему будет добавляться/удаляться стандартный логический атрибут [hidden](http://htmlbook.ru/html/attr/hidden).

<br>

<h3 id="disabled"># $disabled</h3>

Можно добавлять любые логические атрибуты элементам, которые содержатся в компонентах.

Например, давайте добавим специальный логический атрибут ***$disabled***, который добавляет/удаляет стандартный логический атрибут [disabled](http://htmlbook.ru/html/input/disabled) полю ввода:
...
Таким же образом, можно создавать специальные логические атрибуты для любых стандартных логических атрибутов, которые допускаются для элемента в HTML.
...
Мы уже познакомились со свойством объекта компонента [extends](#extends), которое позволяет модифицировать компонент таким образом, чтобы при его монтировании можно было использовать стандартный HTML-элемент.

Одним из существенных недостатков атрибута [is](https://developer.mozilla.org/ru/docs/Web/HTML/Global_attributes/is), который используется при модификации компонентов, представляется тот факт, что он является статическим, а не динамическим.

Это означает, что после монтирования, при изменении значения данного атрибута, например, на название другого компонента, содержимое элемента, в который монтируется компонент, не изменится.

Данную проблему решают т.н. виды, которые определяются с помощью специального атрибута ***$view***.
...
Теперь внесём изменения в компонент Hello, который находится в этом же файле:

```html
<!-- определить компонент Hello  -->
<app-hello>
  <!-- кнопки переключения вида элемента MAIN -->
...
В данном примере, пользовательское свойство **view** содержит текстовое значение, которое представляет название модифицируемого компонента.

Но данное свойство может содержать и объект, который включает в себя обязательное свойство **name**, содержащее название компонента, и необязательное свойство **content**, содержащее HTML-содержимое, передаваемое в слот модифицируемого компонента.
...
Не забудьте добавить Теневой DOM компоненту содержащему слоты и любой из двух уровней инкапсуляции.
...
<app-hello>
  ...
  <!-- присвоить пользовательскому свойству view объект вида,
    чтобы переключить вид элемента MAIN на компонент B
      и передать содержимое свойства content в его слот -->
...
В этом примере мы определили объект для вида компонента B.

Данный объект содержит название компонента и HTML-содержимое в свойстве **content**, которое добавляется в слоты компонента B, при переключении его вида. 
...
Давайте рассмотрим работу пользовательских событий, на примере одного компонента Hello. 

Внесите в него изменения, как показано ниже:
...
Этот простой пример, почти ничем не отличается от [стандартной](https://learn.javascript.ru/dispatch-events) реализации.

Сначала мы назначаем обработчик события для всего компонента, а затем, через секунду, мы вызываем пользовательское событие, используя для этого специальный метод **$event**. В первом аргументе этого метода, ему передаётся название события в виде строки.
...
В этом примере тоже нет ничего необычного, однако, перед тем, как переходить к теме взаимодействия пользовательских событий между различными компонентами, необходимо пояснить, как мы задавали обработчик и вызывали пользовательское событие.

Обратите внимание, что и перед обработчиком, т.е. перед стандартным методом [addEventListener](https://learn.javascript.ru/introduction-browser-events#addeventlistener), и перед специальным методом **$event**, использовалось ключевое слово **this**, как показано ниже:
...
Во всех методах объекта компонента, таких, например, как **connected**, ключевое слово **this** всегда ссылается на сам компонент.

Это означает, что мы задавали обработчик события компоненту Hello, и вызывали для него пользовательское событие *arr-reverse*.
...
Давайте перепишем предыдущий пример, используя этот глобальный объект.

Для этого достаточно лишь заменить ключевое слово **this**, на ключевое слово **document** в обработчике:
...
Поскольку теперь мы используем глобальный объект **document** в качестве источника событий, мы можем легко наладить взаимодействие между различными компонентами.
...
Снова внесите изменения в содержимое компонента Hello, как показано ниже:
...
При нажатии на кнопку, пользовательское свойство **colors**, т.е. массив цветов, передаётся в событие  *arr-reverse* глобального объекта **document**, во втором аргументе метода **$event**, используя для передачи свойство **detail**.

В следующем разделе мы познакомимся с маршрутизатором, в основе которого, так же лежат рассмотренные здесь пользовательские события.
...
Здесь мы рассмотрим маршрутизатор, предоставляемый библиотекой Reacton. Данный маршрутизатор имеет некоторую схожесть с маршрутизатором, который предлагает [Express](https://expressjs.com/ru/guide/routing.html).

В основе маршрутизатора Reacton, лежат рассмотренные ранее [пользовательские](#observer) события, но с некоторыми улучшениями.

Например, как и в Express, название события может представлять собой строку [регулярного](https://developer.mozilla.org/ru/docs/Web/JavaScript/Reference/Global_Objects/RegExp) выражения. Кроме этого, поддерживаются и [параметры](https://metanit.com/web/nodejs/4.15.php) маршрута.

Для объяснения работы маршрутизатора, нам потребуются три простых компонента, которые будут представлять три страницы приложения.
...
Обратите внимание, что все три компонента имеют атрибут ***extends*** со значением "main".

Эти компоненты будут модифицированы для стандартного HTML-элемента MAIN. Маршрутизация в Reacton активно использует [виды](#views).

Теперь добавим меню для этих страниц в компонент Hello, который находится в том же самом файле:
...
Затем, мы добавим HTML-элемент MAIN с атриутом ***$view*** для создания вида, в котором они будут отображаться:
...
Для создания маршрутизатора, используется специальный метод **$router**, в первом аргументе которого, ему передаётся HTML-элемент для отслеживания всех событий-маршрутов или по-другому, просто путей. Все щелчки мышью вне этого HTML-элемента игнорируются маршрутизатором.

Кроме этого, игнорируются и все щелчки не по ссылкам, внутри данного элемента, равно как и ссылки, ведущие на [другие](https://learn.javascript.ru/cross-window-communication#same-origin) источники, т.е. такие, у которых значение свойства **origin**, отличается от значения свойства [location.origin](https://developer.mozilla.org/en-US/docs/Web/API/Location/origin).

Точно так же игнорируются и ссылки на все файлы, внутри элемента маршрутизатора.
...
Во втором аргументе специального метода **$router**, ему передаётся объект с методами, названия которых, представляют пути для отслеживания, а значения, являются обработчиками, выполняемыми при совпадении пути (т.е. названия метода) и свойства [location.href](https://developer.mozilla.org/en-US/docs/Web/API/Location/href).
...
Маршрутизатор начинает срабатывать сразу же при подключении компонента к документу.

Он сравнивает текущее значение свойства **location.href**, со всеми названиями методов, которые мы передали ему во втором аргументе, как показано выше.

Как только маршрутизатор найдёт совпадение значения свойства **location.href** с названием метода, то будет выполнен соответствующий обработчик.

Внутри такого обработчика, мы просто присваиваем пользовательскому свойству **page** название компонента представляющего страницу, а дальше, вступают в работу уже рассмотренные ранее [виды](#views).

Когда мы щёлкаем по ссылкам в меню, вся эта процедура повторяется заново: Поиск-Совпадение-Вид.

Обратите внимание, что для обработчиков маршрутов были использованы стрелочные функции, чтобы ключевое слово **this** внутри них, всегда ссылалось на компонент.

С помощью этого ключевого слова, мы получаем доступ к объекту данных компонента, внутри которого находится свойство **page**.

Как уже было сказано ранее, пути маршрутов можно задавать в виде регулярных выражений в строке.

Давайте изменим последний маршрут, как показано ниже:
...
Подробнее о регулярных выражениях можно почитать [здесь](https://developer.mozilla.org/ru/docs/Web/JavaScript/Reference/Global_Objects/RegExp).

Важно лишь помнить, что при использование символа обратного слэша (\\) в строке маршрута, его необходимо экранировать дважды.
...
Для получения доступа к параметру запроса (не путать с параметрами маршрута), в объект **event** обработчика события маршрута, добавляется свойство [url](https://learn.javascript.ru/url), которое содержит свойство [searchParams](https://learn.javascript.ru/url#searchparams) для работы с параметрами, например, метод **get**.


Последнее, что осталось рассмотреть, это как работать с [параметрами](https://metanit.com/web/nodejs/4.15.php) маршрута, которые не стоит путать с параметрами запроса рассмотренного выше.
...
и создайте два новых компонента в файле *Hello.htm*, как показано ниже:
...
<span id="route-parameters">Параметры маршрута</span> представляют именованные сегменты URL-адреса.

Для получения сегмента в обеих ссылках идущего после начального слэша (/), применяются параметры, которые позволяют задать такому сегменту имя, предваряя его символом двоеточия.
...
значение параметра *:category* будет равняться "phones", а при щелчке по ссылке:

```html
<a href="/tablets">Планшеты</a>
```

параметр *:category* будет иметь значение "tablets".

Кроме этого, можно задавать любое количество параметров в пути запроса.
...
Здесь мы добавляем в компоненты [слоты](https://learn.javascript.ru/slots-composition) и [Теневой DOM](https://learn.javascript.ru/shadow-dom), для работы с этими слотами.
...
Данный обработчик сработает при щелчке по любой из ссылок выше.

Внутри него, мы присваиваем пользовательскому свойству **page** объект с двумя свойствами. Как мы помним, в [виды](#views) можно передавать объекты.

Свойство **name** объекта вида, содержит название компонента, которое определяется из приставки *app-* и значения параметра *:category*.


В свою очередь, свойство объекта вида **content**, передаёт в слот компонента значение параметра *:id*.

Последний аргумент, который можно передать методу **$router**, является объектом и может содержать до семи свойств.

Это свойства: **once**, **capture**, **passive**, **bubbles**, **cancelable**, **composed** и **detail**.
...
Данное свойство не было задействовано в примерах выше, ввиду отсутствия необходимости передавать в обработчики маршрутов пользовательские данные.
...
В этом разделе мы создадим полноценное приложение, используя для этого систему сборки модулей [webpack](https://webpack.js.org/). Выходной файл будет называться *bundle.js* и содержать код всех компонентов и самой библиотеки Reacton.

Само приложение будет выводить список пользователей разных категорий, с возможностью его сортировки и просмотра данных отдельного пользователя.

Создайте новый каталог, например, назовите его *app*.

Затем, откройте терминал из этого каталога, или перейдите в каталог из терминала, и инициируйте новый проект командой:

```
npm init -y
```

Эта команда создаст файл *package.json* с названием, описанием и другими данными проекта.

Кроме этого, данный файл необходим для добавления зависимостей нашего проекта от других пакетов, таких, например, как webpack, с помощью которого мы будем собирать наш проект.

Помимо сборщика модулей, нам потребуются и другие пакеты, в том числе, и для серверного рендеринга, который будет рассмотрен в следующем разделе.

Ниже представлено содержимое готового файла *package.json*.

Скопируйте его и вставьте в свой файл:
...
После выполнения этой команды, в каталоге *app* появится папка *node_modules*, со всеми необходимыми для работы нашего приложения пакетами.

Следующим шагом, будет создание файла конфигурации webpack.
...
Плагин [HtmlWebpackPlugin](https://webpack.js.org/plugins/html-webpack-plugin/) здесь добавляется только для режима разработки.

В режиме итоговой сборки, т.е. в режиме продакшн, он создаёт реальный файл *index.html*, который нам не потребуется, поскольку, для финального приложения, мы будем использовать фреймворк [Express](https://expressjs.com/ru/) и шаблонизатор [Handlebars](https://handlebarsjs.com/).

Теперь давайте создадим папку *src* внутри каталога *app*. В этой папке будет находиться всё исходное содержимое нашего проекта.

Первое, что мы туда добавим, будет файл *index.html*, являющийся главным файлом создаваемого нами приложения.
...
В теге LINK мы подключаем [нормализацию](https://necolas.github.io/normalize.css/) стилей, а само приложение, т.е. главный его компонент, будет модифицирован для его монтирования в элемент BODY.
...
Теперь мы создадим точку входа для сборщика модулей webpack.

Такой точкой у нас будет файл *index.js*, который тоже располагается в папке *src*, вместе со всем остальным исходным содержимым для проекта.
...
Далее, у нас идёт определение примесей для всех компонентов нашего приложения.

Первая примесь представляет собой глоссарий:
...
который необходим для [интернационализации](https://learn.javascript.ru/intl) строк, например, при их сортировке, как будет покано позже.
...
Данный метод будет применяться в компонентах нашего приложения, когда мы будем запускать его на условно-реальном сервере.
...
Последней примесью является [динамически](https://learn.javascript.ru/modules-dynamic-imports) импортированная тестовая база данных:
...
В режиме итоговой сборки, т.е. в режиме продакшн, когда происходит создание файла *bundle.js*, эта база данных в него не попадёт.

Мы просто закомментируем данную строку, поскольку для готового приложения, будет использоваться реальная база данных, а не тестовая.

Давайте создадим эту тестовую базу данных в папке *src* нашего приложения.

Файл тестовой базы данных будет называться *db.js*, создайте этот файл, как показано ниже:
...
Эта база данных представляет собой массив пользователей двух категорий: Дизайнеры и Менеджеры. Теперь пришло время заняться созданием компонентов.

Все наши компоненты будут находиться внутри каталога *components*, расположенного внутри папки *src*. Создайте этот каталог.
...
Главный компонент имеет открытый уровень инкапсуляции и модифицирован для монтирования в элемент BODY:

```html
<app-reacton mode="open" extends="body">
```

В самом начале этого компонента, идёт основная разметка приложения:
...
Все эти три элемента, будут монтировать в себя соответствующие им компоненты.

Кроме этого, элемент MAIN содержит [параметры](#parameters) для передачи в дочерние компоненты пользовательских данных, и является [видом](#views).
...
Главный компонент имеет три пользовательских свойства: **view**, **category** и **id**. Последние два передаются в параметры вида с помощью атрибута ***$params***, как показано ниже:
...
Последний обработчик содержит [параметры](#route-parameters) маршрута для категории и идентификатора пользователя:
...
Теперь давайте добавим три простых компонента в папку *components*. Это будут компоненты: Header, Menu и Footer.

Создайте три файла в этой папке, как показано ниже.
...
В этих компонентах нет ничего особенного, поэтому детально мы их рассматривать не будем.

Обратите лишь внимание на то, что в компоненте Header, мы используем элемент NAV для монтирования в него компонента Menu и, кроме этого, здесь имеется ссылка на статическое изображение логотипа:
...
Для хранения статических файлов, таких, например, как статические изображения или шрифты, используется папка *public*, которую вам надо будет создать в каталоге *app*.

Затем, в папке *public*, создайте подпапку *img*, в которой сохраните логотип библиотеки Reacton под именем [logo.png](https://raw.githubusercontent.com/reacton-js/reacton/main/logo.png), что представлен ниже:
...
Итак, внутри папки *components*, создайте новую подпапку *pages*. В этой папке будут располагаться все компоненты, представляющие страницы приложения.

Кроме этого, всем компонентам страниц, мы будем задавать в имени приставку *page-*, вместо *app-*, как мы делали это для обычных компонентов.
...
Следующие два компонента мы рассмотрим подробнее. Это будут компоненты List и User.

Первый отвечает за вывод списка пользователей из выбранной категории в компоненте Workers, а второй, за вывод информации о конкретном пользователе, выбранном в компоненте List.
...
За вывод массива пользователей из выбранной категории, отвечает цикл [for-of](#for-of) и метод **sort**, который будет рассмотрен далее:
...
Затем, идёт определение пользовательского свойства **users**, которое, в зависимости от наличия примеси **DB**, как показано ниже:
...
среди которых, нас больше всего интересует метод **sort**.

Если значение пользовательского свойства **ascending** является Истиной, то происходит интернациональная сортировка по возрастанию, с учётом выбранной группы, которая содержится в пользовательском свойстве **group**, как показано ниже:
...
Который, в зависимости от выбранного значения в списке, присваивает его пользовательскому свойству **group**, используемому в методе **sort**, как было показано выше.
...
чтобы заголовок содержал значение "Менеджер" или "Дизайнер". После этого, у нас выводится детальная информация о пользователе:
...
В самом конце разметки, мы снова экспортируем объект компонента, который содержит асинхронный метод **data**, внутри которого, мы определяем пользовательськое свойство **user**, как показано ниже:
...
Здесь опять же, в зависимости от наличия примеси **DB**, свойство **user** получит данные либо из тестовой базы данных, либо же из реальной.
...
Откройте окно браузера по адресу [http://localhost:8080/](http://localhost:8080/), в котором будет отображаться наше приложение.

При любом изменении в компонентах, браузер будет автоматически перезагружать страницу.

После того, как мы внесли все необходимые изменения в режиме разработки, перед переносом приложения на сервер, проект необходимо будет запустить в режиме продакшн.
...
которая добавляет в папку *public* скомпонованный файл *bundle.js*, содержащий код библиотеки Reacton и всех компонентов, что мы использовали в проекте.

При этом, сервер по адресу [http://localhost:8080/](http://localhost:8080/) в режиме продакшн не работает. Этот режим создаёт готовый файл *bundle.js* и не предназначен для разработки.

Но перед этим, закомментируйте тестовую базу данных из примеси в файле *index.js*, как показано ниже:
...
Эта база данных используется только для режима разработки, а после того, как наш проект будет скомпонован в режиме продакшн, он уже будет использовать реальную базу данных, которую мы создадим в последнем разделе данного руководства.
...
Это происходит при обращении к серверу поискового бота. В противном случае, когда происходит обращение пользователя, выполняется инструкция *else*, которая выводит [частичное](https://metanit.com/web/nodejs/4.8.php) представление, называемое так же body:
...
Нам осталось создать файл *server.js* в каталоге *app*, который будет содержать всю логику работы нашего условно-реального сервера.
...
Подробно разбирать данный файл не имеет смысла, поскольку он является типичным, при создании сервера на фреймворке Express, обратим лишь внимание на главные моменты.

Во-первых, ниже представлен список основных ботов:
...
Сервер сам определяет, когда к нему обращается бот и отдаёт ему отрендеренное содержимое. Во всех других случаях, он отдаёт содержимое частичного представления.
