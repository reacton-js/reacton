...
Здесь стоит отдельно обратить внимание на актуальное взаимодействие событий и переменных циклов компонента. Даже после обращения массива, для чего введите в консоли браузера команду:

```
hello.$data.colors.reverse()
```

событие всегда будет "отображать" актуальное значение переменной для элемента, в котором данное событие используется.
