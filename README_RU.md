...
Чтобы компоненты можно было монтировать в стандартные HTML-элементы, необходимо использовать свойство **extends** объекта компонента и атрибут ***is*** элемента, в который монтируется компонент. При этом, атрибут ***is*** является статическим, т.е. после изменения его значения, в элемент, которому он принадлежит, другой компонент не сможет примонтироваться.

Эту проблему можно обойти, если связать атрибут ***is*** с пользовательскими данными. Внесите изменения в файл *index.html*, как показано ниже:
...
Для передачи всех шаблонов компонентов в функцию Reacton, задействуется метод [querySelectorAll()](https://learn.javascript.ru/searching-elements-dom#querySelectorAll) и [оператор расширения](https://learn.javascript.ru/rest-parameters-spread-operator#spread-operator):
