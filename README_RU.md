...
*- Добавление раздела [дочерние компоненты](#child-components).*
...
Статическое свойство <span id="tracked-attributes">**attributes**</span> содержит массив с названиями атрибутов при изменении которых, будет вызываться статический метод **changed()**, например:
...
Для передачи данных из родительских компонентов в дочерние, можно использовать [слоты](#slots), [атрибуты](#reactive-attributes) и [события](#events). Слоты и атрибуты позволяют передавать лишь текстовые данные, а с помощью событий, можно обмениваться любыми данными между любыми компонентами, а не только дочерними.
...
Но при изменении значения свойства состояния передаваемого через атрибуты, в дочернем компоненте изменения происходить не будут:
...
Однако, если в конструктор дочернего компонента добавить свойство состояния, которое получает значение из атрибута, и сделать этот атрибут [отслеживаемым](#tracked-attributes), то при изменении состояния в родительском компоненте, будет изменяться состояние и в дочернем.

Внесите изменения в класс компонента SubComponent, как показано ниже:

```js
exports = class {
  constructor(props) {
    // присвоить свойству состояния значение из атрибута
    this.color = props.color
  }

  static mode = 'open' // добавить Теневой DOM

  static attributes = ['color'] // отслеживаемый атрибут

  // вызывается при изменении отслеживаемого атрибута
  static changed(name, oldValue, newValue) {
    // присвоить свойству состояния новое значение из атрибута
    this.color = newValue
  }
}
```

Теперь внесите изменения в стили этого компонента, чтобы передавать не значение атрибута ***color*** напрямую:

```js
color: {{ $props.color }};
```

а значение свойства состояния **color** дочернего компонента:
...
Теперь цвет заголовка в дочернем компоненте изменится на зелёный, если ввести в консоли браузера следующую команду:
